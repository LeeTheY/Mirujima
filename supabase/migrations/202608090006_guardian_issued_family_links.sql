-- Family codes are intentionally directional: a guardian issues and a student redeems.
-- Active links are preserved; legacy pending codes issued by students cannot be redeemed.
...
begin
  ...
  select role into current_role from public.profiles where id = p_actor_user_id;
  if current_role is distinct from 'guardian' then raise exception 'guardian role required'; end if;
...
begin
  ...
  if p_code_hash is null or p_code_hash !~ '^[0-9a-f]{64}$' then raise exception 'invalid code hash'; end if;
  select role into current_role from public.profiles where id = p_actor_user_id;
  if current_role is distinct from 'student' then raise exception 'student role required'; end if;
  ...
  if exists (select 1 from public.family_links where student_user_id = p_actor_user_id and guardian_user_id = pending_link.guardian_user_id and status = 'active') then
    raise exception 'family link already exists';
  end if;
