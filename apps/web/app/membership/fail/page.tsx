...
export default async function MembershipFailPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <main className="payment-page"><header><Brand /></header><section className="payment-card result-card"><div className="test-mode-banner"><strong>테스트 결제</strong><span>실제 청구 없음</span></div><h1>Premium 결제가 완료되지 않았습니다.</h1><p>{paymentFailureCopy(code)}</p><p className="muted">결제가 승인되지 않아 멤버십과 데이터는 변경되지 않았습니다.</p><Link className="button" href="/membership/checkout">다시 시도하기</Link></section></main>;
}
