...
export async function GET(request: Request) {
  ...
  if (!code) return NextResponse.redirect(new URL("/onboarding?error=oauth", url.origin));
  const supabase = await createClient();
  const { error } = await supabase.auth.exchangeCodeForSession(code);
  if (error) return NextResponse.redirect(new URL("/onboarding?error=oauth", url.origin));
